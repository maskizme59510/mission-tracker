...
import { getTeamMemberIds } from "@/lib/rbac";
...
export default async function TeamMissionsPage() {
  const { supabase, user, profile } = await requireUserSessionWithProfile();
  ...
  if (teamMemberIds.length === 0) {
    return (
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold text-slate-900">Missions de l&apos;equipe</h2>
        <p className="text-slate-600">Aucun commercial rattache.</p>
      </section>
    );
  }
  if (!supabaseAdmin) {
  ...
  const [{ data: healthRows, error: healthError }, { data: missionsMeta, error: metaError }, { data: profilesData, error: profileError }] =
    await Promise.all([
      supabaseAdmin.from("mission_health_view").select("mission_id,owner_id,consultant_first_name,consultant_last_name,client_name,start_date,follow_up_frequency_days,latest_report_date,next_followup_date").in("owner_id", teamMemberIds),
      supabaseAdmin.from("missions").select("id,owner_id,tjm,cj,consultant_type,next_followup_date"),
      supabaseAdmin.from("profiles").select("id,first_name,last_name,user_code").in("id", teamMemberIds),
    ]);
  ...
  const byCommercial = missions.reduce<Record<string, typeof missions>>((acc, mission) => {
    const owner = profileById.get(mission.owner_id);
    const commercialLabel = owner ? `${owner.first_name} ${owner.last_name}${owner.user_code ? ` (${owner.user_code})` : ""}` : "Commercial inconnu";
    if (!acc[commercialLabel]) acc[commercialLabel] = [];
    acc[commercialLabel].push(mission);
    return acc;
  }, {});
  const sortedCommercials = Object.keys(byCommercial).sort((a, b) => a.localeCompare(b, "fr", { sensitivity: "base" }));
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900">Missions de l&apos;equipe</h2>
          <p className="mt-1 text-slate-600">Missions de vos commerciaux rattachés.</p>
        </div>
      ...
      {sortedCommercials.map((commercial) => {
        const byClient = byCommercial[commercial].reduce<Record<string, typeof missions>>((acc, mission) => {
        ...
        return (
          <article key={commercial} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-900">{commercial}</h3>
