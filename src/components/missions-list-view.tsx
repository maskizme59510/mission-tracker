...
export type MissionsListMissionRow = {
  mission_id: string;
...
export type MissionsListMarginRow = {
  id: string;
  tjm: number | null;
  cj: number | null;
...
function getPlanningBadge(mission: MissionsListMissionRow): { label: string; classes: string } | null {
...
function getMarginBadge(row: MissionsListMarginRow | undefined) {
...
type MissionsListViewProps = {
  missions: MissionsListMissionRow[];
  marginsByMissionId: Map<string, MissionsListMarginRow>;
  listTitle: string;
};

export function MissionsListView({ missions, marginsByMissionId, listTitle }: Readonly<MissionsListViewProps>) {
  const missionsByClient = missions.reduce<Record<string, MissionsListMissionRow[]>>((accumulator, mission) => {
              ...
              <div className="space-y-2 p-3">
                {missionsByClient[clientName]
                  ...
                  .map((mission) => {
                    ...
                    const marginBadge = getMarginBadge(marginsByMissionId.get(mission.mission_id));
